...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    //  сюди вантажимо всі збережені дані
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Unlock", style: .plain, target: self, action: #selector(authenticateUser))
        
        loadData()
        lockApp()   // щоб при запуску одразу був "Locked"
    }
    ...
    @objc func authenticateUser() {
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            let reason = "Please authenticate to see your photos."
            ...
            let ac = UIAlertController(title: "Biometry unavailable", message: "Your device is not configured for Face ID / Touch ID.", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
    ...
    // Метод для розблокування:
    func unlockApp() {
        people = allPeople
        collectionView.reloadData()
        title = "Unlocked!"
        
        // Тепер замість Unlock показуємо кнопку Done (щоб заблокувати)
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Lock", style: .plain, target: self, action: #selector(lockApp))
    }
    ...
    // Збереження/завантаження масиву
    func saveData() {
        let encoder = JSONEncoder()
        if let savedData = try? encoder.encode(allPeople) {
    ...
    func loadData() {
        if let savedData = UserDefaults.standard.data(forKey: "people") {
            ...
            if let loadedPeople = try? decoder.decode([Person].self, from: savedData) {
